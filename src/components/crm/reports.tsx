...
type ReportType = 'clients' | 'proposals' | 'summary'
...
type ReportResponse = ClientReportResponse | ProposalReportResponse | SummaryReportResponse
...
function FilterPanel({
  ...
  onFiltersChange: (filters: { clientId: string; status: string; startDate: string; endDate: string }) => void
  onGenerate: () => void
  ...
  const showClientFilter = reportType === 'proposals'
  const showStatusFilter = reportType === 'proposals'
                ...
                <div className="space-y-2">
                  ...
                  >
                    ...
                    <SelectContent>
                      ...
                      <SelectItem value="In Evaluation">
                        In Evaluation
                      </SelectItem>
      ...
      <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-4">
        ...
        <Card>
          <CardContent className="p-4">
            ...
            <p className="mt-1 text-2xl font-bold text-foreground">
              {formatCurrency(data.summary.totalProposalValue)}
            </p>
          </CardContent>
        </Card>
      </div>
      ...
      <Card>
        <CardHeader className="flex flex-row items-center justify-between space-y-0 pb-4">
          <div>
            ...
            <CardDescription>
              {data.totalRecords} client{data.totalRecords !== 1 ? 's' : ''} found
            </CardDescription>
          </div>
          <div className="flex items-center gap-2">
            <Button
              variant="outline"
              size="sm"
              onClick={() => exportToCSV(csvData, 'client-report.csv')}
            >
            ...
            <Table>
              ...
              <TableBody>
                {data.data.map((client) => {
                  const proposalCount =
                    client.proposals?.length ?? client._count?.proposals ?? 0
                  const totalValue =
                    client.proposals?.reduce((sum, p) => sum + p.value, 0) ?? 0
                  return (
                ...
                <TableRow>
                  ...
                  <TableCell className="text-right font-semibold">
                    {data.data.reduce(
                      (sum, c) =>
                        sum +
                        (c.proposals?.length ?? c._count?.proposals ?? 0),
                      0
                    )}
                  </TableCell>
                  <TableCell className="text-right font-semibold">
      ...
      <div className="grid gap-4 sm:grid-cols-3">
        ...
        <Card>
          <CardContent className="p-4">
            ...
            <p className="mt-1 text-2xl font-bold text-foreground">
              {Object.keys(data.summary.byClient).length}
            </p>
          </CardContent>
        </Card>
      </div>
      ...
      <Card>
        <CardHeader className="flex flex-row items-center justify-between space-y-0 pb-4">
          <div>
            ...
            <CardDescription>
              {data.totalRecords} proposal{data.totalRecords !== 1 ? 's' : ''}{' '}
              found
            </CardDescription>
          </div>
          <div className="flex items-center gap-2">
            <Button
              variant="outline"
              size="sm"
              onClick={() => exportToCSV(csvData, 'proposal-report.csv')}
            >
              <Download className="mr-2 h-3.5 w-3.5" />
              CSV
            </Button>
...
function SummaryReportView({ data }: { data: SummaryReportResponse }) {
  ...
  // Pie chart data: proposals by status
  ...
  // Bar chart data: total value by status
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-4">
        ...
        <Card>
          <CardContent className="p-4">
            ...
            <p className="mt-1 text-2xl font-bold text-foreground">
              {formatCurrency(data.summary.proposals.totalFilteredValue)}
            </p>
          </CardContent>
        </Card>
      </div>
      ...
      <div className="grid gap-6 lg:grid-cols-2">
        ...
        {/* Bar Chart - Total Value by Status */}
        <Card>
          <CardHeader>
            <CardTitle className="text-base">
        ...
        <CardHeader className="flex flex-row items-center justify-between space-y-0 pb-4">
          ...
          <div className="flex items-center gap-2">
            <Button
              variant="outline"
              size="sm"
              onClick={() => exportToCSV(csvData, 'summary-report.csv')}
            >
              <Download className="mr-2 h-3.5 w-3.5" />
              CSV
            </Button>
...
export default function ReportsPage() {
  ...
  const handleReportTypeChange = (type: ReportType) => {
    setReportType(type)
    setGenerated(false)
    setFilters({ clientId: '', status: '', startDate: '', endDate: '' })
  }
      ...
      {isLoading || isFetching ? (
        ...
        <Card>
          <CardContent className="flex flex-col items-center justify-center py-16">
            ...
            <p className="text-sm font-medium text-muted-foreground">
              Select a report type and click &quot;Generate Report&quot; to get
              started.
            </p>
          </CardContent>
        </Card>
